...
mysql -se 'INSERT INTO clanpoints_history SELECT UNIX_TIMESTAMP(), id, points FROM clans WHERE period_id = 8;'

# Ensure forum users are associated properly with their clans.
mysql -se 'DELETE FROM forumuser_clans; INSERT IGNORE INTO forumuser_clans SELECT phpbb3_user_group.user_id AS user_id, clans.id AS clan_id, clans.name AS clan_name FROM phpbb3_user_group INNER JOIN clans ON phpbb3_user_group.group_id = clans.forum_group_id AND clans.period_id = 8;'
