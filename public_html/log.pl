...
if (my $id = $c->param('log_id')) {
	...
	if (!$private) {
		...
		print $c->format_log_lines([$log_line], [qw/period clan team member/], 0);
	} else {
		print $c->h2("Log line $id");
	...
	$log_lines = $c->get_log_lines_member($c->{member_info}->{id}, $start, $count+1);
	$private = $c->is_clan_member($c->{clan_info}{id});
	$cols = [qw/team/];
	print $c->h2("Log for ".$c->render_member($c->{member_info}));
} elsif ($c->{team_info}) {
	$log_lines = $c->get_log_lines_team($c->{team_info}->{id}, $start, $count+1);
	$private = $c->is_clan_member($c->{clan_info}{id});
	$cols = [qw/member/];
	print $c->h2("Log for ".$c->render_team($c->{team_info}));
} elsif ($c->{clan_info}) {
	$log_lines = $c->get_log_lines_clan($c->{clan_info}->{id}, $start, $count+1);
	$category = 'clan';
	$private = $c->is_clan_member($c->{clan_info}{id});
	$cols = [qw/team member/];
	print $c->h2("Log for ".$c->render_clan($c->{clan_info}));
} else {
	$log_lines = $c->get_log_lines_period($c->{period_info}->{id}, $start, $count+1);
	$category = 'period';
	$private = 0;
