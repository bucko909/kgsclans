...
if (!empty($topic_data['poll_start']))
{
	...
	if ($forum_id == 3) {
		...
		if ($clan_leader_group_id) {
			...
			$voters_bucko = array();
			while ($row = $db->sql_fetchrow($result)) {
				$voter_bucko[$row['username']] = 1;
				$row['poll_option_text'] = str_replace('.', '', $row['poll_option_text']);
				$voters_bucko[] = $row['username'];
				$voted_for_bucko[] = '"'.$row['poll_option_text'].'"';
			}
			if (count($voter_bucko) == 1) {
				$voted_for_bucko_str = join(' and ', $voted_for_bucko);
			} else {
				foreach ($voters_bucko as $key => $value) {
					$voted_for_bucko[$key] .= ' ['.$value.']';
				}
				$voted_for_bucko_str = join(' and ', $voted_for_bucko);
			}
			$template->assign_vars(array(
...
$sql = $db->sql_build_query('SELECT', array(
	'SELECT'	=> 'u.*, z.friend, z.foe, p.*, c.clan_id, c.clan_name',
	...
	'LEFT_JOIN'	=> array(
		...
		array(
			'FROM'	=> array('forumuser_clans' => 'c'),
			'ON'	=> 'c.user_id = p.poster_id'
		),
	),
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'CLAN_ID'			=> $user_cache[$poster_id]['clan_id'],
		'CLAN_NAME'			=> $user_cache[$poster_id]['clan_name'],
